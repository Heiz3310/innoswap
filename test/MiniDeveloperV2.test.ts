...
describe("MiniDeveloperV2", function () {
  before(async function () {
    await prepare(this, ["MiniDeveloperV2", "InnovaiToken", "ERC20Mock", "RewarderMock", "RewarderBrokenMock"])
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [
      ...
      ["chef", this.MiniDeveloperV2, [this.innovai.address]],
    ...
    await this.innovai.mint(this.chef.address, getBigNumber(10000))
    await this.lp.approve(this.chef.address, getBigNumber(10))
    await this.chef.setInnovaiPerSecond("10000000000000000")
  ...
  describe("PendingInnovai", function () {
    it("PendingInnovai should equal ExpectedInnovai", async function () {
      ...
      let expectedInnovai = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
      let pendingInnovai = await this.chef.pendingInnovai(0, this.alice.address)
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef.updatePool(0))
        ...
        .withArgs(
          ...
          (await this.chef.poolInfo(0)).accInnovaiPerShare
        )
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of INNAI and reward", async function () {
      ...
      let timestamp = (await ethers.provider.getBlock(log.blockNumber)).timestamp
      let expectedInnovai = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
      expect((await this.chef.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedInnovai)
