...
describe("MasterDeveloperV2", function () {
  before(async function () {
    await prepare(this, ["MasterDeveloper", "InnovaiToken", "ERC20Mock", "MasterDeveloperV2", "RewarderMock", "RewarderBrokenMock"])
    await deploy(this, [["brokenRewarder", this.RewarderBrokenMock]])
  })

  beforeEach(async function () {
    await deploy(this, [["innovai", this.InnovaiToken]])

    await deploy(this, [
      ["lp", this.ERC20Mock, ["LP Token", "LPT", getBigNumber(10)]],
      ["dummy", this.ERC20Mock, ["Dummy", "DummyT", getBigNumber(10)]],
      ["chef", this.MasterDeveloper, [this.innovai.address, this.bob.address, getBigNumber(100), "0", "0"]],
    ])
    ...
    await deploy(this, [
      ["chef2", this.MasterDeveloperV2, [this.chef.address, this.innovai.address, 1]],
  ...
  describe("PendingInnovai", function () {
    it("PendingInnovai should equal ExpectedInnovai", async function () {
      await this.chef2.add(10, this.rlp.address, this.rewarder.address)
      ...
      await advanceBlock()
      let expectedInnovai = getBigNumber(100)
        .mul(log2.blockNumber + 1 - log.blockNumber)
        .div(2)
      let pendingInnovai = await this.chef2.pendingInnovai(0, this.alice.address)
      expect(pendingInnovai).to.be.equal(expectedInnovai)
    })
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef2.updatePool(0))
        ...
        .withArgs(
          ...
          (await this.chef2.poolInfo(0)).accInnovaiPerShare
        )
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of INNAI and reward", async function () {
      ...
      await this.chef2.harvestFromMasterDeveloper()
      let log2 = await this.chef2.withdraw(0, getBigNumber(1), this.alice.address)
      let expectedInnovai = getBigNumber(100)
        .mul(log2.blockNumber - log.blockNumber)
        .div(2)
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedInnovai)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.innovai.balanceOf(this.alice.address))
        .to.be.equal(await this.r.balanceOf(this.alice.address))
        .to.be.equal(expectedInnovai)
    })
    ...
    it("Harvest for INNAI-only pool", async function () {
      ...
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedInnovai)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.innovai.balanceOf(this.alice.address)).to.be.equal(expectedInnovai)
    })
  })
