...
// File @innoswap/breadbox-sdk/contracts/IBatchFlashBorrower.sol@v1.0.1
// License-Identifier: MIT
...
// File @innoswap/breadbox-sdk/contracts/IFlashBorrower.sol@v1.0.1
// License-Identifier: MIT
...
// File @innoswap/breadbox-sdk/contracts/IStrategy.sol@v1.0.1
// License-Identifier: MIT
...
// File @innoswap/breadbox-sdk/contracts/IBreadBoxV1.sol@v1.0.1
// License-Identifier: MIT

interface IBreadBoxV1 {
...
interface ISwapper {
    /// @notice Withdraws 'amountFrom' of token 'from' from the BreadBox account for this swapper.
    /// Swaps it for at least 'amountToMin' of token 'to'.
    /// Transfers the swapped tokens of 'to' into the BreadBox using a plain ERC20 transfer.
    /// Returns the amount of tokens 'to' transferred to BreadBox.
    /// (The BreadBox skim function will be used by the caller to get the swapped funds).
    function swap(
        IERC20 fromToken,
        IERC20 toToken,
        address recipient,
    ...
    /// Withdraws 'amountFrom' of token 'from' from the BreadBox account for this swapper.
    /// Swaps it for exactly 'exactAmountTo' of token 'to'.
    /// Transfers the swapped tokens of 'to' into the BreadBox using a plain ERC20 transfer.
    /// Transfers allocated, but unused 'from' tokens within the BreadBox to 'refundTo' (amountFromMax - amountFrom).
    /// Returns the amount of 'from' tokens withdrawn from BreadBox (amountFrom).
    /// (The BreadBox skim function will be used by the caller to get the swapped funds).
    function swapExact(
        IERC20 fromToken,
        IERC20 toToken,
...
// File contracts/NovaPair.sol
// License-Identifier: UNLICENSED
// Nova Lending Medium Risk

/// @title NovaPair
/// @dev This contract allows contract calls to any contract (except BreadBox)
/// from arbitrary callers thus, don't trust calls from this contract in any circumstances.
contract NovaPairMediumRiskV1 is ERC20, BoringOwnable, IMasterContract {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    ...
    IBreadBoxV1 public immutable breadBox;
    NovaPairMediumRiskV1 public immutable masterContract;
    ...
    Rebase public totalAsset; // elastic = BreadBox shares held by the NovaPair, base = Total fractions held by asset suppliers
    ...
    function name() external view returns (string memory) {
        return string(abi.encodePacked("Nova Medium Risk ", collateral.safeName(), "/", asset.safeName(), "-", oracle.name(oracleData)));
    }
    ...
    // Settings for the Medium Risk NovaPair
    ...
    constructor(IBreadBoxV1 breadBox_) public {
        breadBox = breadBox_;
        masterContract = this;
        feeTo = msg.sender;
    }
    ...
    function init(bytes calldata data) public payable override {
        require(address(collateral) == address(0), "NovaPair: already initialized");
        (collateral, asset, oracle, oracleData) = abi.decode(data, (IERC20, IERC20, IOracle, bytes));
        require(address(collateral) != address(0), "NovaPair: bad pair");
    ...
    function accrue() public {
        ...
        uint256 fullAssetAmount = breadBox.toAmount(asset, _totalAsset.elastic, false).add(_totalBorrow.elastic);
    ...
    modifier solvent() {
        _;
        require(_isSolvent(msg.sender, false, exchangeRate), "NovaPair: user insolvent");
    }
    ...
    /// False if tokens from msg.sender in `breadBox` should be transferred.
    function _addTokens(
        IERC20 token,
        uint256 share,
        uint256 total,
        ...
        if (skim) {
            require(share <= breadBox.balanceOf(token, address(this)).sub(total), "NovaPair: Skim too much");
        } else {
            breadBox.transfer(token, msg.sender, address(this), share);
        }
    }
    ...
    function addCollateral(
        ...
        emit LogAddCollateral(skim ? address(breadBox) : msg.sender, to, share);
    }
    ...
    function _removeCollateral(address to, uint256 share) internal {
        ...
        breadBox.transfer(collateral, address(this), to, share);
    }
    ...
    function _addAsset(
        ...
        uint256 allShare = _totalAsset.elastic + breadBox.toShare(asset, totalBorrow.elastic, true);
        ...
        emit LogAddAsset(skim ? address(breadBox) : msg.sender, to, share, fraction);
    }
    ...
    function _removeAsset(address to, uint256 fraction) internal returns (uint256 share) {
        ...
        require(_totalAsset.base >= 1000, "Nova: below minimum");
        totalAsset = _totalAsset;
        emit LogRemoveAsset(msg.sender, to, share, fraction);
        breadBox.transfer(asset, address(this), to, share);
    }
    ...
    function _borrow(address to, uint256 amount) internal returns (uint256 part, uint256 share) {
        ...
        share = breadBox.toShare(asset, amount, false);
        Rebase memory _totalAsset = totalAsset;
        require(_totalAsset.base >= 1000, "Nova: below minimum");
        _totalAsset.elastic = _totalAsset.elastic.sub(share.to128());
        totalAsset = _totalAsset;
    ...
    function _repay(
        ...
        uint256 share = breadBox.toShare(asset, amount, true);
        uint128 totalShare = totalAsset.elastic;
        _addTokens(asset, share, uint256(totalShare), skim);
        totalAsset.elastic = totalShare.add(share.to128());
        emit LogRepay(skim ? address(breadBox) : msg.sender, to, amount, part);
    }
    ...
    // Function on BreadBox
    uint8 internal constant ACTION_BREAD_DEPOSIT = 20;
    uint8 internal constant ACTION_BREAD_WITHDRAW = 21;
    uint8 internal constant ACTION_BREAD_TRANSFER = 22;
    uint8 internal constant ACTION_BREAD_TRANSFER_MULTIPLE = 23;
    uint8 internal constant ACTION_BREAD_SETAPPROVAL = 24;

    // Any external call (except to BreadBox)
    ...
    /// @dev Helper function for depositing into `breadBox`.
    function _breadDeposit(
        bytes memory data,
        uint256 value,
        uint256 value1,
        uint256 value2
        ...
        return breadBox.deposit{value: value}(token, msg.sender, to, uint256(amount), uint256(share));
    }

    /// @dev Helper function to withdraw from the `breadBox`.
    function _breadWithdraw(
        bytes memory data,
        uint256 value1,
        uint256 value2
    ) internal returns (uint256, uint256) {
        (IERC20 token, address to, int256 amount, int256 share) = abi.decode(data, (IERC20, address, int256, int256));
        return breadBox.withdraw(token, msg.sender, to, _num(amount, value1, value2), _num(share, value1, value2));
    }

    /// @dev Helper function to perform a contract call and eventually extracting revert messages on failure.
    /// Calls to `breadBox` are not allowed for obvious security reasons.
    ...
    function _call(
        ...
        require(callee != address(breadBox) && callee != address(this), "NovaPair: can't call");

        (bool success, bytes memory returnData) = callee.call{value: value}(callData);
        require(success, "NovaPair: call failed");
    ...
    /// Only applicable to `ACTION_CALL`, `ACTION_BREAD_DEPOSIT`.
    ...
    function cook(
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action == ACTION_ADD_COLLATERAL) {
                ...
                require((!must_update || updated) && rate > minRate && (maxRate == 0 || rate > maxRate), "NovaPair: rate not ok");
            } else if (action == ACTION_BREAD_SETAPPROVAL) {
                (address user, address _masterContract, bool approved, uint8 v, bytes32 r, bytes32 s) =
                    abi.decode(datas[i], (address, address, bool, uint8, bytes32, bytes32));
                breadBox.setMasterContractApproval(user, _masterContract, approved, v, r, s);
            } else if (action == ACTION_BREAD_DEPOSIT) {
                (value1, value2) = _breadDeposit(datas[i], values[i], value1, value2);
            } else if (action == ACTION_BREAD_WITHDRAW) {
                (value1, value2) = _breadWithdraw(datas[i], value1, value2);
            } else if (action == ACTION_BREAD_TRANSFER) {
                (IERC20 token, address to, int256 share) = abi.decode(datas[i], (IERC20, address, int256));
                breadBox.transfer(token, msg.sender, to, _num(share, value1, value2));
            } else if (action == ACTION_BREAD_TRANSFER_MULTIPLE) {
                (IERC20 token, address[] memory tos, uint256[] memory shares) = abi.decode(datas[i], (IERC20, address[], uint256[]));
                breadBox.transferMultiple(token, msg.sender, tos, shares);
                ...
                value1 = breadBox.toShare(asset, totalBorrow.toElastic(_num(part, value1, value2), true), true);
    ...
    function liquidate(
        ...
        Rebase memory _totalBorrow = totalBorrow;
        Rebase memory breadBoxTotals = breadBox.totals(collateral);
        ...
        require(allBorrowAmount != 0, "NovaPair: all are solvent");
        ...
        uint256 allBorrowShare = breadBox.toShare(asset, allBorrowAmount, true);

        if (!open) {
            // Closed liquidation using a pre-approved swapper for the benefit of the LPs
            require(masterContract.swappers(swapper), "NovaPair: Invalid swapper");

            // Swaps the users' collateral for the borrowed asset
            breadBox.transfer(collateral, address(this), address(swapper), allCollateralShare);
            swapper.swap(collateral, asset, address(this), allBorrowShare, allCollateralShare);

            uint256 returnedShare = breadBox.balanceOf(asset, address(this)).sub(uint256(totalAsset.elastic));
            ...
            breadBox.transfer(asset, address(this), masterContract.feeTo(), feeShare);
            ...
            breadBox.transfer(collateral, address(this), swapper == ISwapper(0) ? to : address(swapper), allCollateralShare);
            if (swapper != ISwapper(0)) {
                swapper.swap(collateral, asset, msg.sender, allBorrowShare, allCollateralShare);
            }

            breadBox.transfer(asset, msg.sender, address(this), allBorrowShare);
