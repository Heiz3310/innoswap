...
contract MasterDeveloperV2 is BoringOwnable, BoringBatchable {
    ...
    IMasterDeveloper public immutable MASTER_DEVELOPER;
    ...
    uint256 private constant MASTERDEVELOPER_INNAI_PER_BLOCK = 1e20;
    ...
    event LogUpdatePool(uint256 indexed pid, uint64 lastRewardBlock, uint256 lpSupply, uint256 accInnovaiPerShare);
    event LogInit();

    /// @param _MASTER_DEVELOPER The InnoSwap MCV1 contract address.
    /// @param _innovai The INNAI token contract address.
    /// @param _MASTER_PID The pool ID of the dummy token on the base MCV1 contract.
    constructor(IMasterDeveloper _MASTER_DEVELOPER, IERC20 _innovai, uint256 _MASTER_PID) public {
        MASTER_DEVELOPER = _MASTER_DEVELOPER;
        INNAI = _innovai;
        MASTER_PID = _MASTER_PID;
    }

    /// @notice Deposits a dummy token to `MASTER_DEVELOPER` MCV1. This is required because MCV1 holds the minting rights for INNAI.
    /// Any balance of transaction sender in `dummyToken` is transferred.
    /// The allocation point for the pool on MCV1 is the total allocation point for all pools that receive double incentives.
    /// @param dummyToken The address of the ERC-20 token to deposit into MCV1.
    function init(IERC20 dummyToken) external {
        uint256 balance = dummyToken.balanceOf(msg.sender);
        require(balance != 0, "MasterDeveloperV2: Balance must exceed 0");
        dummyToken.safeTransferFrom(msg.sender, address(this), balance);
        dummyToken.approve(address(MASTER_DEVELOPER), balance);
        MASTER_DEVELOPER.deposit(MASTER_PID, balance);
        emit LogInit();
    }
    ...
    function pendingInnovai(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 innovaiReward = blocks.mul(innovaiPerBlock()).mul(pool.allocPoint) / totalAllocPoint;
    ...
    /// @notice Calculates and returns the `amount` of INNAI per block.
    function innovaiPerBlock() public view returns (uint256 amount) {
        amount = uint256(MASTERDEVELOPER_INNAI_PER_BLOCK)
            .mul(MASTER_DEVELOPER.poolInfo(MASTER_PID).allocPoint) / MASTER_DEVELOPER.totalAllocPoint();
    }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            poolInfo[pid] = pool;
            emit LogUpdatePool(pid, pool.lastRewardBlock, lpSupply, pool.accInnovaiPerShare);
        }
    }
    ...
    /// @notice Harvests INNAI from `MASTER_DEVELOPER` MCV1 and pool `MASTER_PID` to this MCV2 contract.
    function harvestFromMasterDeveloper() public {
        MASTER_DEVELOPER.deposit(MASTER_PID, 0);
    }
