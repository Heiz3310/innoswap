...
contract CloneRewarderTime is IRewarder,  BoringOwnable{
    ...
    address public immutable MASTERDEVELOPER_V2;
    ...
    constructor (address _MASTERDEVELOPER_V2) public {
    ...
    function onInnovaiReward (uint256 pid, address _user, address to, uint256, uint256 lpTokenAmount) onlyMCV2 lock override external {
        require(IMasterDeveloperV2(MASTERDEVELOPER_V2).lpToken(pid) == masterLpToken);
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        uint256 lpSupply = IMasterDeveloperV2(MASTERDEVELOPER_V2).lpToken(_pid).balanceOf(MASTERDEVELOPER_V2);
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 time = block.timestamp.sub(pool.lastRewardTime);
            uint256 innovaiReward = time.mul(rewardPerSecond);
            accToken1PerShare = accToken1PerShare.add(innovaiReward.mul(ACC_TOKEN_PRECISION) / lpSupply);
        }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            uint256 lpSupply = IMasterDeveloperV2(MASTERDEVELOPER_V2).lpToken(pid).balanceOf(MASTERDEVELOPER_V2);

            if (lpSupply > 0) {
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                uint256 innovaiReward = time.mul(rewardPerSecond);
                pool.accToken1PerShare = pool.accToken1PerShare.add((innovaiReward.mul(ACC_TOKEN_PRECISION) / lpSupply).to128());
            }
