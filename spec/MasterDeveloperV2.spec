/*
 * This is a specification file for MasterDeveloperV2's formal verification
 * using the Certora prover.
 *
 * Run this file with scripts/_runMasterDeveloperV2.sh
 */
...
using DummyINNAI as innovaiToken
...
methods {
	...
	poolInfoAccInnovaiPerShare(uint256 pid) returns (uint128) envfree
	...
	// INNAI token
	INNAI() returns (address) envfree

	// Rewarder
	onInnovaiReward(uint256, address, address, uint256, uint256) => NONDET
...
rule monotonicityOfAccInnovaiPerShare(uint256 pid, method f) {
	env e;

	uint128 _poolInfoAccInnovaiPerShare = poolInfoAccInnovaiPerShare(pid);

	calldataarg args;
	f(e, args);

	uint128 poolInfoAccInnovaiPerShare_ = poolInfoAccInnovaiPerShare(pid);

	assert compareUint128(poolInfoAccInnovaiPerShare_, _poolInfoAccInnovaiPerShare);
}
...
rule noChangeToOtherPool(uint256 pid, uint256 otherPid) {
	...
	uint128 _otherAccInnovaiPerShare = poolInfoAccInnovaiPerShare(otherPid);
	...
	uint128 otherAccInnovaiPerShare_ = poolInfoAccInnovaiPerShare(otherPid);
	uint64 otherLastRewardBlock_ = poolInfoLastRewardBlock(otherPid);
	uint64 otherAllocPoint_ = poolInfoAllocPoint(otherPid);

	assert(_otherAccInnovaiPerShare == otherAccInnovaiPerShare_, "accInnovaiPerShare changed");
...
rule preserveTotalAssetOfUser(method f, uint256 pid, address user,
	...
	require INNAI() != lpToken(pid); // <-- check this again (Nurit)
...
rule innovaiGivenInHarvestEqualsPendingInnovai(uint256 pid, address user, address to) {
	env e;

	require to == user && user != currentContract && e.msg.sender == user;
	require innovaiToken == INNAI();

	uint256 userInnovaiBalance = innovaiToken.balanceOf(e, user);
	uint256 userPendingInnovai = pendingInnovai(e, pid, user);

	// Does success return value matters? Check with Nurit
	harvest(e, pid, to);

	uint256 userInnovaiBalance_ = innovaiToken.balanceOf(e, user);

	assert(userInnovaiBalance_ == (userInnovaiBalance + userPendingInnovai),
		   "pending innovai not equal to the innovai given in harvest");
}
...
function callFunctionWithParams(method f, uint256 pid, address sender, address to) {
	...
	} else if (f.selector == pendingInnovai(uint256, address).selector) {
		pendingInnovai(e, pid, to);
