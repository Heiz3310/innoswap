...
const { MINIDEVELOPER_ADDRESS } = require("@innoswap/sdk");
...
task("migrate", "Migrates liquidity from Uniswap to InnoSwap")
  .addOptionalParam(
    "a",
    "Token A",
...
task("masterchef:add", "Add pool to masterchef").setAction(async function (
  ...
  await (
    await masterDeveloper
      .connect(await getNamedSigner("dev"))
      .add(1000, "0x3e78a806b127c02b54419191571d9379819e989c", true)
  ).wait();
});

task("masterchef:deposit", "MasterDeveloper deposit")
  ...
  .setAction(async function (
    ...
    const { lpToken } = await masterDeveloper.poolInfo(pid);

    await run("erc20:approve", { token: lpToken, spender: masterDeveloper.address });

    await (
      await masterDeveloper.connect(await getNamedSigner("dev")).deposit(pid, amount)
    ).wait();
  });

task("masterchef:withdraw", "MasterDeveloper withdraw")
  .addParam("pid", "Pool ID")
  .addParam("amount", "Amount")
  .setAction(async function (
    { pid, amount },
...
task("generator:enter", "InnovaeGenerator enter")
  .addParam("amount", "Amount")
  .setAction(async function (
    { amount },
    { ethers: { getNamedSigner } },
    ...
    await run("erc20:approve", { token: innovai.address, spender: generator.address });

    await (await generator.connect(await getNamedSigner("dev")).enter(amount)).wait();
  });

task("generator:leave", "InnovaeGenerator leave")
  .addParam("amount", "Amount")
  .setAction(async function (
    { amount },
    { ethers: { getNamedSigner } },
    ...
    await (await generator.connect(await getNamedSigner("dev")).leave(amount)).wait();
  });

task("spawner:serve", "InnovaeGenerator serve")
  .addParam("a", "Token A")
  .addParam("b", "Token B")
  .setAction(async function (
    { a, b },
...
task("deploy:complex-rewarder", "Deploy ComplexRewarder")
  ...
  .setAction(async function (
    ...
    let miniDeveloperAddress;

    if (chainId === "31337") {
      miniDeveloperAddress = (await deployments.get("MiniDeveloperV2")).address;
    } else if (chainId in MINIDEVELOPER_ADDRESS) {
      miniDeveloperAddress = MINIDEVELOPER_ADDRESS[chainId];
    } else {
      throw Error("No MINIDEVELOPER!");
    }

    const { address } = await deploy("ComplexRewarderTime", {
      from: deployer,
      args: [rewardToken, 0, miniDeveloperAddress],
      log: true,
